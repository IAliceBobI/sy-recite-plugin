...
- **Word-level cloze** — beyond block-level targets you can circle words: select a span in the original and hit "Cloze" — the picked words get a bold yellow highlight on the spot (a placeholder, still readable), then get masked on extraction into yellow-dashed blanks in the paper; fill them back from memory in the write slot right after the block — a cloze question. One block can carry several blanks, each masked on its own (leave at least one character between two); select an existing blank and hit "Cloze" again to undo. In the compare document the left column stays masked, with a "dug words" answer line underneath — right vs. wrong at a glance
...
- **Collect into a composition** — practice ends with a piece of your own: hit "Collect" on the extract bar and everything you wrote in the write slots is pulled into a standalone document — items joined in order, blank lines between them, scaffolding like prompts and context left out. The result is a plain sub-document "仿写文·<title>" under the source document, editable and exportable — an essay of your own; collecting again overwrites the same document in place, and "Delete practice" never deletes it
...
2. Select the passages you want to practice and hit "Drill This" on the recite bar — a prompt slot appears right after the target: write one line on what the passage says (that's the question) or leave it empty for pure recall; the rest of the original stays as context. Only drilling a few words? Select a span and hit "Cloze" — extraction turns it into a cloze fill-in. Not sure how to break the piece into beats? Click "AI Split" (Pro) for a head start — splitting is itself practice, so write your own prompts once you get the hang of it
...
|---|---|---|
| 🎯 Practice loop | Target practice with context carried along, word-level cloze, extraction, per-item compare & clear, compare document, dictation check, association practice, collect into composition, flashcard companion | — |
